...
## Overview
In Phase 2, we enhanced the grasp planner to be more robust and practical. The goal was to ensure that generated grasps are not only theoretically stable (force closure) but also **reachable** by the robot's arm and **collision-free**. We also added ranking to prioritize the best grasps.

## What We Did

### 1. Enhanced Service Definition
We updated `PlanGrasp.srv` to support more complex queries:
- **New Inputs**:
...
We integrated `VirtualRobot::DifferentialIK` into the service.
...
We integrated `VirtualRobot::CDManager` (Collision Detection Manager).
...
### 4. Grasp Ranking
- We implemented logic to sort the valid grasps based on their quality score (Simox grasp quality).
...
- **Solution**: We implemented unit conversion helpers. When receiving the `object_pose` from ROS (meters), we convert it to Simox format (millimeters) before planning. When returning grasp poses to ROS, we convert back to meters.
...
- **Solution**: We adjusted the IK parameters (steps, loops) and ensured the initial configuration was reasonable. We also accepted that not all grasps are reachable and simply filtered those out.

### Data Type Conversion
- **Issue**: Converting between ROS `geometry_msgs::Pose` and Eigen matrices (used by Simox) was repetitive and error-prone.
- **Solution**: We created a `conversion_helpers.hpp` file with `rosPoseToEigen` and `eigenToRosPose` functions to handle this cleanly and consistently.
